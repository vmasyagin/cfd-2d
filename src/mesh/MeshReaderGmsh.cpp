...
void MeshReaderGmsh::read(Grid* g)
{
	...
	std::memset(cfi, 0, g->cCount*sizeof(int));
	for (int i = 0; i < g->cCount; i++)
	{
		for (int j = 0; j < 3; j++)
		{
			...
			if (p != -1)
			{
				...
				// центр ребра
				g->edges[iEdge].c[0].x = 0.5*(g->nodes[g->edges[iEdge].n1].x + g->nodes[g->edges[iEdge].n2].x);
				g->edges[iEdge].c[0].y = 0.5*(g->nodes[g->edges[iEdge].n1].y + g->nodes[g->edges[iEdge].n2].y);
