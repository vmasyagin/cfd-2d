...
void FEM_DG_IMPLICIT::init(char * xmlFileName)
{
	...
	solverMtx->init(&(grid), MATR_DIM, 1);
	log("Solver type: %s.\n", solverMtx->getName());
...
void FEM_DG_IMPLICIT::calcDiffusionIntegral() {
    ...
    double FR, FU, FV, FE;
    double **mx7 = allocMtx7();
    double **my7 = allocMtx7();
    ...
    for (int iCell = 0; iCell < grid.cCount; iCell++) {
        ...
        for (int iGP = 0; iGP < GP_CELL_COUNT; iGP++) {
            ...
            calcJ(mx7, par.r, par.u, 1.0, par.v, 0.0, par.ML, fTAU_XX, fTAU_XY, fTAU_YY);
            calcJ(my7, par.r, par.u, 0.0, par.v, 1.0, par.ML, fTAU_XX, fTAU_XY, fTAU_YY);
            for (int i = 0; i < FIELD_COUNT_EXT; i++) {
                for (int j = 0; j < FIELD_COUNT_EXT; j++) {
                    for (int ii = 0; ii < BASE_FUNC_COUNT; ii++) {
                        for (int jj = 0; jj < BASE_FUNC_COUNT; jj++) {
                            matrSmall[ii][jj] = mx7[i][j] * getDfDx(ii, iCell, p);
                            matrSmall[ii][jj] += my7[i][j] * getDfDy(ii, iCell, p);
        ...
        if (c2 >= 0) {
			...
			for (int iBF = 0; iBF < BASE_FUNC_COUNT; iBF++) {
				...
				for (int iGP = 0; iGP < GP_EDGE_COUNT; iGP++) {
					...
					mat1.getML(par);
					getTensorComponents(fTAU_XX, fTAU_XY, fTAU_YY, c1, p);

					FS2 = fTAU_XX * n.x + fTAU_XY * n.y;
					FS3 = fTAU_XY * n.x + fTAU_YY * n.y;
					FS4 = (fTAU_XX * par.u + fTAU_XY * par.v) * n.x + (fTAU_XY * par.u + fTAU_YY * par.v) * n.y;
					FS5 = par.ML * (4. * par.u / 3. * n.x - 2. * par.v / 3. * n.y);
					FS6 = par.ML * ((par.v - 2. * par.u / 3.) * n.x + (par.u - 2. * par.v / 3.) * n.y);
					FS7 = par.ML * (-2. * par.u / 3. * n.x + 4. * par.v / 3. * n.y);
