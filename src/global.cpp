...
void Material::getML(Param &par) {
    par.ML = ML;
//    if ( !m_bMixt )	// åñëè íå ñìåñü
//    {
//        Component &rComp = g_pComponent[ m_iComponent ];
//
//        if ( m_eML == ML_CONST )
//        {
//            rV.ML = rComp.ML;
//        } else {
//            rV.ML = rComp.ML * sqrt( _pow_3_( rV.T / rComp.T0 ) ) * ( rComp.T0 + rComp.TS ) / ( rV.T + rComp.TS );
//        } // if

//    } else {		// ñìåñü ãàçîâ
//
//        REAL fML = 0.0;
//        REAL fM_ = 0.0;		//  ôîðìóëû   M_ = 1 / M   ãäå   M = 1 / SUM( Ci / Mi )
//
//        if ( m_eML == ML_CONST )
//        {
//            for ( INDEX i=0; i<g_iComponentCount; i++ )
//            {
//                Component &rComp = g_pComponent[ i ];
//                REAL fCK_M = rV.CK[ i ] / rComp.M;
//                fM_ += fCK_M;
//                fML += fCK_M * rComp.ML;
//            } // for(i)
//
...
//                REAL fCK_M = rV.CK[ i ] / rComp.M;
//                fM_ += fCK_M;
//                fML += fCK_M * rComp.ML * sqrt( _pow_3_( rV.T / rComp.T0 ) ) * ( rComp.T0 + rComp.TS ) / ( rV.T + rComp.TS );
//            } // for(i)
//
//            rV.ML = fML / fM_;
//
